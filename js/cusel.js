...
var cuselFrame='<div class="cusel '+chElClass+' '+classDisCusel+'"'+' id=cuselFrame-'+chElId+' style="width: auto"'+' tabindex="'+chElTab+'"'+'>'+'<div class="cuselFrameRight"></div>'+'<div class="cuselText">'+defaultText+'</div>'+'<div class="cusel-scroll-wrap"><div class="cusel-scroll-pane" id="cusel-scroll-'+chElId+'">'+
...
{cuselWrap.css("display","none");}}
else if(clicked.is("span")&&clicked.parents(".cusel-scroll-wrap").is("div")&&(!(clicked.hasClass("group"))))
...
{jQuery(this).parent().find(".cuselOptHover").eq(0).removeClass("cuselOptHover");});var arrCusel=jQuery(".cusel"),colCusel=arrCusel.length-1; for(i=0;i<=colCusel;i++)
{arrCusel.eq(i).css("z-index",colCusel-i+2);}});}
...
if(settings.scrollbarOnLeft){cssToApply.paddingLeft=settings.scrollbarMargin+settings.scrollbarWidth+'px';}else{cssToApply.paddingRight=/*settings.scrollbarMargin+*/'0px';}
...
{   
    $target=$(e.target);if($target.is('a')){var h=$target.attr('href');if(h.substr(0,1)=='#'){scrollTo(h);}}});*/
